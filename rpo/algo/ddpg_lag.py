...
class DDPG_LA(object):
    ...
    def run(self, logger=None, eval=True):
        ...
        while t < self.max_epochs:
            ...
            episode_max_eq_viol = max(np.abs(eq_viol).max(), episode_max_eq_viol)
            episode_max_ineq_viol = max(ineq_viol.max(), episode_max_ineq_viol)

            # if ineq_viol.max() < 1e-6:
            #     print("update:", self.env.update(state, full=True))
            #     print("ineq:", ineq_viol)
            #     print("action", action)
            #     raise Exception
