...
class DemandLoader(object):
    ...
    def reset(self):
        ...
        self.cache = np.concatenate((pd, qd), axis=1) / self.baseMVA
    ...
    def process(self, ps):
        ...
        nnz = len(idx)
        p_total = self.bus[:, idx_bus.PD].sum() * self.T * 1.0
    ...
    def fetch(self):
        ...
        tmp = np.zeros_like(self.cache[0]) if done else self.cache[self.counter].copy()
        self.counter += 1
