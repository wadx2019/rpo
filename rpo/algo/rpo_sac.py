...
from rpo.algo.agent import PDSAC_PA
...
class RPOSAC(object):
    ...
    def __init__(self, env, work_dir, name, logger, automatic_entropy_tuning=True, alpha=0.2, max_steps=10, embed_dim=256, hidden_dim=256,
        ...
        self.agent = PDSAC_PA(
                            ...
                            box_constraint=BoxConstraint(*self.env.box_constraint_partial,
                                                         device=device, volatile=self.env.volatile, update=self.env.update),
                            alpha=alpha,
                            lr_alpha=lr_alpha, lr_actor=lr_actor, lr_critic=lr_critic, lr_dual=lr_dual,
